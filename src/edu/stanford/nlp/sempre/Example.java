...
public class Example {
  ...
  public String oriToken(int i) {return languageInfo.oriTokens.get(i); }
  ...
  public String nerTag(int i) { return languageInfo.nerTags.get(i); }
  public String phrase(int start, int end) { return languageInfo.phrase(start, end); }
  public String oriPhrase(int start, int end) { return languageInfo.oriPhrase(start, end); }
  ...
  public String expandQuery(String query) {
	  ...
	  for (String pre_query: this.entity_map.keySet()) {
		  if (pre_query.equals(query))
			  return query;
		  if (pre_query.contains(query) && TokenLevelMatchFeatures.diffSetSize(pre_query, query) < 4) {
			  if (expandedQuery != null) {
				  if (EntityLinkingResult.tokenContains(expandedQuery, pre_query))
					  continue;
				  else if (EntityLinkingResult.tokenContains(pre_query, expandedQuery))
					  expandedQuery = pre_query;
				  else
					  return null;
			  }				  
			  else
  ...
  public void addQuery(String query) {
	  this.entity_map.put(query, query);
  }
  
  public void addExpansion(String query, String expansion) {
	  this.entity_map.put(query.toLowerCase(), expansion.toLowerCase());
  }
  
  public void fillEntities(List<IntPair> entities) {
	  int numToken = numTokens();
	  String[] tmp = {"NNP", "NNPS"};
	  Set<String> etPosTags = new HashSet<String>(Arrays.asList(tmp));
	  boolean[] pool = new boolean[numToken];
	  for (IntPair entity: entities) {
		  for (int i = entity.st; i < entity.ed; i ++)
			  pool[i] = true;
	  }
	  for (int i = 0; i < numTokens(); i++) {
		  if (!pool[i] && etPosTags.contains(this.posTag(i))) {
			  this.languageInfo.posTags.set(i, "NULL");
		  } else if (pool[i]) {
			  this.languageInfo.posTags.set(i, "NNP");
		  }
	  }
  }
}

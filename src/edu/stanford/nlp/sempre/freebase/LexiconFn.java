...
public class LexiconFn extends SemanticFn {
  ...
  private String entity_type;  // normal_entity or concept 
  ...
  public void init(LispTree tree) {
    ...
    for (int i = 1; i < tree.children.size(); i++) {
      ...
      else if (value.equals("entity")) { this.mode = "entity"; this.entity_type = "normal_entity"; }
      ...
      else if (value.equals("concept")) this.entity_type = "concept";
  ...
  public static void getEntityEntryFeatures(LexicalEntry.EntityLexicalEntry eEntry, FeatureVector features,
    ...
    if (FeatureExtractor.containsDomain("entityAlignmentFeature"))  {
    	features.add("entityAlignmentFeature", "entity.popularity", Math.log(eEntry.getPopularity() + 1));
    	features.add("entityAlignmentFeature", "entity.IDLevel", eEntry.getIDLevel());
    	features.add("entityAlignmentFeature", "entity.distance", eEntry.getDistance());
    }
  }
...
//        LogInfo.logs("entity = [%s], type = [%s], types = [%s]", eEntry.textDescription, eEntry.type, eEntry.types);
        break;
      case "unary":
        // Unaries
...
//           LogInfo.log("LexiconFn: querying for entity: " + query);
          ...
          if (entity_type.equals("concept"))
        	  entries = lexicon.lookupConcepts(query, entitySearchStrategy, ex);
          else
        	  entries = lexicon.lookupEntities(query, entitySearchStrategy, ex);
